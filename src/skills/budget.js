...
var skillsSpi = require('../skills-spi.js');

function BudgetService(spi) {
	this.spi = spi;
}
BudgetService.prototype.donnerBudgetInitial = function(bot, message) { 

  this.spi.database.child('initialDays').once('value').then(function (days) {
...
BudgetService.prototype.initialiserBudget = function(bot, message) { 
    ...
    convo.ask(`Est ce que vous voulez mettre à jour le budget initial de ProxiLabs a ${days} jours, pour cette année ?`, [
      {
        ...
        callback: function (response, convo) {
          this.spi.database.child('initialDays').set(days).then(function () {
...
BudgetService.prototype.rappelerCodeProjet = function(bot, message) { 
